...
from math_agent.nodes.sensitivity import (
    ...
    sensitivity_interpret_node, sensitivity_code_generate_node,
)
...
def test_sensitivity_generate_skips_llm_without_flag(mocker, workdir):
    ...
    assert delta["sensitivity_phase"] == "done"
    assert any(
        e == "sensitivity: LLM generate disabled; put source/inject/sensitivity.py or --allow-coder-llm"
        for e in delta["errors"]
    )


def test_sensitivity_generate_uses_inject_file(mocker, workdir):
    data = workdir / "source"
    inject = data / "inject"
    inject.mkdir(parents=True)
    (inject / "sensitivity.py").write_text(
        "print('RESULT: parameter=K values=[1] results=[2]')\n",
        encoding="utf-8",
    )
    state = _primary_state(workdir, "K = 0.18\nprint('RESULT: T_max=1')\n")
    state.data_dir = str(data)
    state.sensitivity_plan_dump = SensitivityPlan(runs=[
        {"parameter": "K", "values": [0.12, 0.18, 0.24], "metric": "T_max"},
    ]).model_dump()
    spy = mocker.patch("math_agent.nodes.sensitivity.complete")
