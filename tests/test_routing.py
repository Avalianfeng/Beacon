...
def test_routing_low_score_stops():
    assert after_model_critic(_state_with("basic", 4, 0)) == "stop"


def test_routing_low_score_stops_even_at_old_cap():
    assert after_model_critic(_state_with("basic", 4, 3)) == "stop"
...
def test_routing_final_low_score_stops():
    assert after_model_critic(_state_with("final", 4, 0)) == "stop"


def test_routing_no_model_critic_stops():
    s = MathModelingState(problem="x", stage_target="basic")
    assert after_model_critic(s) == "stop"
...
def test_after_paper_critic_stops_when_below_threshold():
...
def test_after_paper_critic_takeover_advances_when_paper_complete():
    s = _state_with_paper_critic(score=4, approved=False, writer_iter=0)
    s.paper_review_takeover = True
    assert after_paper_critic(s) == "advance"


def test_after_paper_critic_takeover_does_not_advance_incomplete():
    s = _state_with_paper_critic(score=4, approved=False, writer_iter=0, complete=False)
    s.paper_review_takeover = True
    assert after_paper_critic(s) == "stop"


def test_after_paper_critic_stops_when_incomplete_paper():
    s = _state_with_paper_critic(score=4, approved=False, writer_iter=0, complete=False)
    assert after_paper_critic(s) == "stop"


def test_after_paper_critic_stops_when_no_critic():
    s = MathModelingState(problem="p")
    s.paper.abstract = "非空论文"
    s.paper.model_section = "m"
    s.paper.solution = "s"
    s.paper.conclusion = "c"
    assert after_paper_critic(s) == "stop"


def test_after_paper_critic_stops_empty_paper():
    s = MathModelingState(problem="p")
    assert after_paper_critic(s) == "stop"
...
def test_blueprint_critic_stops_when_not_approved():
    s = _state_with_blueprint_critic(approved=False, iteration=1)
    assert after_blueprint_critic(s) == "stop"


def test_blueprint_critic_stops_when_no_report():
...
def test_consistency_stops_when_not_approved():
    s = _state_with_consistency_report(approved=False, score=4, iteration=1)
    assert after_model_code_consistency(s) == "stop"


def test_consistency_stops_when_approved_but_low_score():
    """approved=True 但 score < 门槛仍不 advance。"""
    s = _state_with_consistency_report(approved=True, score=5, iteration=1)
    assert after_model_code_consistency(s) == "stop"


def test_consistency_stops_without_primary():
    s = _state_with_consistency_report(approved=False, score=0, iteration=1)
    assert after_model_code_consistency(s) == "stop"


def test_consistency_stops_when_no_reports():
...
def test_blueprint_critic_stops_on_coverage_gap():
