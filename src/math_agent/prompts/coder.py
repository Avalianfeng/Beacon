...
SYSTEM = (
    ...
    "**数值健壮性（硬性要求）**：统计/拟合指标必须防御退化输入——"
    "分组样本数<2、分母为 0（如 SS_tot=0）、空数组等场景用保护分支处理"
    "（输出合理占位值或跳过该指标并注明），禁止裸除零产生 RuntimeWarning/NaN 后崩溃；"
    "所有输出数值必须是有限数。\n"
    "**注释纪律（硬性要求）**：代码无需任何行内注释；如需说明，只能在功能块上方写一行中文注释；"
    "注释内禁止出现字面量 \\n、\\t 等转义序列——需要换行就写真正的换行。"
    "一旦把 \\n 写进注释，同一物理行 # 之后的语句会被吞进注释导致 NameError/SyntaxError。\n"
