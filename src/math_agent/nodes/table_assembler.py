...
_FORBIDDEN_PATTERNS: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"扫参参数名必须用[^。\n]*"), ""),
    (re.compile(r"\s*[（(]\s*cost_shock\s*[)）]"), ""),
    (re.compile(r"\s*[（(]\s*markup_p\s*[)）]"), ""),
    (re.compile(r"\s*[（(]\s*loss_scale\s*[)）]"), ""),
    (re.compile(r"(?<![A-Za-z0-9_])cost_shock(?![A-Za-z0-9_])"), "成本冲击"),
    (re.compile(r"(?<![A-Za-z0-9_])markup_p(?![A-Za-z0-9_])"), "加成分位"),
    (re.compile(r"(?<![A-Za-z0-9_])loss_scale(?![A-Za-z0-9_])"), "损耗率缩放"),
    (re.compile(r"(?<![A-Za-z0-9_])q2_week_profit(?![A-Za-z0-9_])"), "一周收益代理"),
