...
完整时间顺序与角色切换：**只读** [全流程定稿 00](全流程分析/全流程定稿/00-正式闭环.md)（正式流程已锁：入口是 `run --plan`，不是交卷通行证）。下面是五幕摘要。

| 幕 | 做什么 | 定稿 |
|----|--------|------|
| 一 · 方向 | 导入 → EDA → 外/本地独立探索 → Diff → 人拍板 | [01–09](全流程分析/全流程定稿/README.md) |
| 二 · 算明白 | 研究者编码、实验、出数（**≠ 交卷**） | [10](全流程分析/全流程定稿/10-研究会话.md) |
| 三 · 手柄 | `brief.json` 定稿，驾驭图 | [11](全流程分析/全流程定稿/11-brief定稿.md) |
| 四 · 启动图 | 预检 → 登记插座 → **`run --plan` 出论文** | [12–15](全流程分析/全流程定稿/15-启动图.md) |
| 五 · 收口 | critic→手改循环；人评 → `accept` | [16–17](全流程分析/全流程定稿/16-停机交棒.md) |
