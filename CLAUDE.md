...
- **工作日志**：进度与结论只写 `docs/实现计划-8-20/log/YYYY-MM-DD-简要说明.md`（同一天多主题可多文件）。只允许 pwsh `Add-Content` 追加（新文件自动创建）；禁止用编辑/写入工具改日志；写前不必读。新文件首行须带六态 frontmatter（`type=log status=active updated=日期`）。`log.md` 已冻结。`实际操作记录/` 是外部模型原始输出，只读。
