...
### P15 字面量 `\n` 吞语句复发（r6 批5 首发，r7 首轮复现）

- **现象**：r6 批5 出现 `# ...必须加钢带。\nuse_steel_A = ...`（注释内字面量 `\n` 吞掉赋值语句 → NameError）；r7 首轮以**代码区形态**复发：`control_B = '压陷' if ... else '粘结')\nuse_steel_A = ...`（`\n` 出现在语句之间，无 `#`）→ SyntaxError。
- **根因（事实）**：模型生成 JSON 代码草稿时把换行转义成字面量 `\n` 两个字符；此前的提示词只禁止"整段 code 写成转义字符串"，未覆盖注释内/语句间零散 `\n`，也无机械兜底。
- **解决方案（已落地，按用户方案 + fail-fast 兜底）**：
  1. 提示词（主图 prompt + coder SYSTEM）：代码无需任何行内注释；如需说明只能在功能块上方写一行中文注释；注释内禁止字面量 `\n`/`\t`，需要换行就写真正的换行；
  2. 确定性兜底 `runner.detect_literal_backslash_n`：最小词法扫描（识别单/双/三引号字符串），字符串之外任何字面量 `\n`（注释内或代码区）在写入执行前即拒绝，附明确反馈进入既有重试通道；不做静默改写（字符串内如 `'锚杆直径\n/mm'` 不受影响）。
- **验证**：检测器 4 项单测（两种失败形态命中、字符串/正常代码/三引号/行延续不误报）+ 提示词 2 项 + 节点"不执行即拒绝"1 项；全量 693 passed / 4 skipped。
- **说明**：修复对**正在运行的 r7 不生效**（worker 已加载旧代码），除非其 supervisor 恢复重启 worker；r8 及以后生效。
- **commit**：`d07a3ac`

### P16 门禁错误分级（L1/L2/L3）—— 效率与质量的平衡设计

- **背景（用户设计决策）**：既有逻辑是"极强约束 + 多次重试"达成目的。r8 出现"同一失败原因连续 4 次"告警（字面量 `\n` 反复生成）后确认：机械噪声类错误走"拒绝→重试"是死循环，模型改不好它。用户提出：本地环节可适当降低严格程度，对非原则性错误"点出但不拒绝"，把分类变成体系设计逻辑的一部分，实现效率与质量的平衡。
- **分级设计（已确定为系统设计逻辑，下文所有门禁按此分级）**：

| 级别 | 定义 | 处理机制 | 成员（已观测） |
|---|---|---|---|
| **L1 原则性**（硬拒绝，永不放行） | 数据真实性/证据血缘/输出契约层面的错误；后期阶段（paper_critic 等 LLM 评审）抓伪造不可靠，放行=产出错误论文的风险 | 确定性门禁拒绝 + 定向反馈重试 | 未读取附件（数据血缘）、硬编码结果、exit-0 失败声明、RESULT 指标契约违反、nan/inf 出现在证据行、figure 无 png、全零/越界数值 |
| **L2 机械噪声**（自动修复，零 LLM 成本） | 语法/词法层面、模型反复生成且修复无歧义的错误；拒绝重试性价比最低 | 确定性自动修复后直接执行 | 字符串外字面量 `\n`（`normalize_literal_backslash_n`，36c7dc5）、import 缺漏/拼写（`_auto_fix_imports`）、plt.show 阻塞（`_strip_dangerous_calls`） |
| **L3 质量类**（点出 + 定向修复，不静默放行） | 可运行但数学/数值/实现不正确的错误；评审能给出实质反馈，低分预算内收敛 | 一致性评审打分 <8 → 低分修复通道（3 轮预算）携带 issues/suggestions 定向修复 | K 回归方向转录错误、安全裕度公式无物理意义、子问题未实现、硬编码参数未读表、量纲混乱 |

- **边界原则**：L1 不因效率放松（真实性与血缘是不可协商的底线）；L2 不做"拒绝"（能修就修）；L3 不静默放行（必须带着审查意见回到生成，否则错误会一路流进论文）。
