...
- ⚠️ **[`内外全流程总纲.md`](内外全流程总纲.md)**：**端到端全流程（2026-08-27，D-007）**——内（操作面 S0–S9）+ 外（D 盘工作面 S0–S7）+ 教训回流（诊断 §6 规则 → Beacon 闸门）一张图；b 数据先探 / c 图表辅助 / d 领域知识注入升级为强制产物；题目档案补赛后段（赛后诊断/教训卡片/calibration/关联题）。
